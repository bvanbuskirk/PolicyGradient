...
class PGAgent(BaseAgent):
    ...
    def train(self, observations, actions, rewards_list, next_observations, terminals):
        ...
        # TODO: DONE update the PG actor/policy using the given batch of data 
    ...
    def estimate_advantage(self, obs: np.ndarray, rews_list: np.ndarray, q_values: np.ndarray, terminals: np.ndarray):
            ...
            if self.gae_lambda is not None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    A{t} = delta{t} + gamma*lambda*A{t+1}
                      where A{t} = sum_{t'=t}^T(gamma*lambda)^(t'-t) * delta{t'}
                      and delta{t'} = r{t'} + gamma*V{t'+1} - V{t'}
                    '''
        ...
        if self.standardize_advantages:
          means = np.mean(advantages)
          stds = np.std(advantages)
          advantages = normalize(advantages, means, stds)
        return advantages
    ...
    def _discounted_return(self, rewards):
        ...
        gammas *= np.array(rewards)
        list_of_discounted_returns = np.sum(gammas) * np.ones(len(rewards))
    ...
    def _discounted_cumsum(self, rewards):
        ...
        list_of_discounted_cumsums = [np.sum(gammas[:len(rewards)-i] * rewards[i:]) for i in range(len(rewards))]
